      ...
      <el-table-column label="操作" width="160">
        <template slot-scope="scope" prop="sysStatus">
          <el-button type="primary" :disabled="scope.row.status != 0" size="small"
                     @click.native="$router.push({name: 'addStaticPage', params: {data: scope.row, type: 'view'}})">查看
          </el-button>
          <el-button type="danger" :disabled="scope.row.status != 0" size="small"
                     @click.native="$router.push({name: 'addStaticPage', params: {data: scope.row, type: 'amend'}})">修改
          </el-button>
        </template>
      </el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      addNews() {
        this.$router.push('addStaticPage')
      },
      modification(data) {
